...
def build_forward(H, x, p1_x, p2_x, p3_x, p4_x, p5_x, p6_x, p7_x, p8_x, f_x, phase, reuse):
    ...
    cnn = tf.concat(0, (next_frame[0], next_frame[1], next_frame[2], next_frame[3], next_frame[4], next_frame[5], next_frame[6], next_frame[7], next_frame[8]))
    
    cnn = tf.transpose(cnn, perm = [1, 2, 3, 0])
    print "cnn.shape %s" % str(cnn.get_shape())
    with tf.variable_scope("conv_1x1", reuse=reuse):
        c_w = tf.get_variable('c_w', shape=[1, 1, 9, 1], initializer=tf.random_normal_initializer(stddev=0.01))
