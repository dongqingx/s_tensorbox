...
def build_forward(H, x, p_x, pp_x, f_x, phase, reuse):
    ...
    '''
    cnn = tf.reshape(cnn,
                     [H['batch_size'] * H['grid_width'] * H['grid_height'], H['later_feat_channels'] * 3])
    initializer = tf.random_uniform_initializer(-0.1, 0.1)
    with tf.variable_scope('decoder', reuse=reuse, initializer=initializer):
        scale_down = 0.01
        lstm_input = tf.reshape(cnn * scale_down, (H['batch_size'] * grid_size, H['later_feat_channels'] * 3))
        if H['use_lstm']:
...
def train(H, test_images):
    ...
    for phase in ['train', 'test']:
        ...
        q[phase] = tf.FIFOQueue(capacity=100, dtypes=dtypes, shapes=shapes)
